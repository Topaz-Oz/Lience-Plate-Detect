...
def load_models():
    try:
        ...
        recognizer = YOLO('../models/recognizer.pt')
        province_detector = YOLO('../models/province.pt')
        logging.info("All models loaded successfully")
        return detector, recognizer, province_detector
...
def main():
    try:
        ...
        detector, recognizer, province_detector = load_models()
                    ...
                    try:
                        ...
                        for cc in range(0, 2):
                            for ct in range(0, 2):
                                ...
                                # Use recognizer to read plate characters
                                char_results = recognizer.predict(
                                    source=rotated_img,
                                    conf=0.25,
                                    iou=0.45,
                                    max_det=20
                                ...
                                if len(char_results.boxes) > 0:
                                    ...
                                    if plate_number != "unknown":
                                        # Detect province
                                        province_results = province_detector.predict(
                                            source=rotated_img,
                                            conf=0.25,
                                            iou=0.45,
                                            max_det=1
                                        ...
                                        if len(province_results.boxes) > 0:
                                            province_cls = int(province_results.boxes.cls[0].item())
                                            province_name = province_detector.names[province_cls]
                                            plate_number = f"{plate_number} ({province_name})"

                                        list_read_plates.add(plate_number)
